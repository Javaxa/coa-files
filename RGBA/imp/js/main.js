...
document.addEventListener('DOMContentLoaded', function() {
    const acaCutoffButton = document.getElementById('acaCutoff');
    const elementSelect = document.getElementById('elementSelect');
    
    function updateAcaTooltip() {
        const selectedElement = elementSelect.value;
        const elementInfo = elementPricesData.find(el => el.symbol === selectedElement);
        
        if (elementInfo && elementInfo.aca) {
            acaCutoffButton.setAttribute('data-tooltip', `Average Crustal Abundance (${elementInfo.aca} ppm)`);
        } else {
            acaCutoffButton.setAttribute('data-tooltip', 'ACA value not available for this element');
        }
    }

    // Update tooltip when page loads
    updateAcaTooltip();

    // Update tooltip when element selection changes
    elementSelect.addEventListener('change', updateAcaTooltip);
    
    acaCutoffButton.addEventListener('click', function() {
        const selectedElement = elementSelect.value;
        const elementInfo = elementPricesData.find(el => el.symbol === selectedElement);
        
        if (elementInfo && elementInfo.aca) {
            document.getElementById('minCutoff').value = elementInfo.aca;
            applyAcaCutoff();
        } else {
            alert('ACA value not available for the selected element.');
        }
    });
});

function applyAcaCutoff() {
    const minCutoff = document.getElementById('minCutoff').value;
    if (minCutoff) {
        updateMap();
        updateElementTable();
